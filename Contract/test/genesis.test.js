...
describe('Genesis', () => {
  ...
  before(async () => {
    ...
    this.genesis = await genesis.deploy("genesis", "genesis", "https://gladiators-metadata-api.herokuapp.com/api/token")
  ...
  it('setBaseURI function succeeds', async () => {
    await this.genesis.connect(this.nftOwner).setBaseURI("https://gladiators-metadata-api.herokuapp.com/api/token")

    expect(
      (await this.genesis.baseTokenURI())
    ).to.equal("https://gladiators-metadata-api.herokuapp.com/api/token")
  })
